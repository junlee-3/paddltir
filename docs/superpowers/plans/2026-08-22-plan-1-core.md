...
```
(The `Paddler`/`Roster` types referenced here are created in Task 2; this file will not compile until then — that is expected. Keep it out of the build until Task 2 by creating it in Task 2's Step 1 instead if the build complains. Simplest: create this file now but **only** with `fixturesURL()` and `SeededRNG`; add `makePaddler`/`standardMixedRoster` in Task 2.)
...
@Suite struct GreedyTests {
    ...
    @Test func balancesWeightTightly() {
        ...
        #expect(r.metrics.sideMismatches <= 1)
    }
    ...
    @Test func smallBoatOpen() {
        ...
        #expect(r.metrics.seated == 10 && r.metrics.totalPower == 6580)   // top-10 ergs: 680,670,660,655,650,645,640,635,630,625
    ...
    @Test func prefersFewerMovesFromCurrent() {
        ...
        // Same optimum quality as base, but whichever tie it picks must not be worse than base on every other component.
        #expect(!(base.isFull && Scoring.evaluate(base, roster: roster, reference: current).lexKey < r.metrics.lexKey))
        #expect(r.metrics.moves != nil)
    }
...
public enum Greedy {
    public static func autoFill(_ req: PlacementRequest) -> PlacementResult {
        ...
        let selection = Selection.select(capacity: boat.capacity, locked: lockedIDs,
                                         candidates: req.candidates, roster: roster, rule: req.rule)
...
```
(Remove the unused `bestAction` variable once the code compiles cleanly — it is shown only to make the shape obvious. Final code must have no warnings.)
...
@Suite struct SuggestionsTests {
    ...
    @Test func findsImprovingSwapsSortedBest() {
        ...
        #expect(s.first!.improves == .weight || s.first!.improves == .side || s.first!.improves == .seat)
    }
...
```
Note on `"rule": null`: `decodeIfPresent` treats JSON `null` as nil — fine. `Metrics.moves` is `Int?` and encodes as absent when nil (default Codable synthesis for optionals omits nil? No — synthesized `Codable` *encodes* `"moves": null`). To keep the JSON clean, add to `Metrics` a custom `encode(to:)` that uses `encodeIfPresent` for `moves` (copy the synthesized keys into a `CodingKeys` enum). Do this in Task 10 when the first fixture is written.
